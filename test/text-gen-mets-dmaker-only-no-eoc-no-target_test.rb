...
class TestTextGenMetsDmakerOnlyNoEocNoTarget < MiniTest::Test

  COMMAND = 'ruby bin/text-gen-mets-dmaker-only-no-eoc-no-target.rb'

  VALID_TEXT          = 'test/fixtures/texts/dmaker-only-no-eoc-no-target'
  EMPTY_TEXT          = 'test/fixtures/texts/empty-dir'
  DMAKER_ONLY_TEXT    = 'test/fixtures/texts/dmaker-only-no-eoc-no-target'
  CANONICAL_DMAKER_ONLY_XML = 'test/fixtures/canonical/valid_mets-dmaker-only-no-eoc-no-target.xml'
