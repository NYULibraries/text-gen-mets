...
class TestTextGenMetsNoTarget <  Minitest::Test

  COMMAND = 'ruby bin/text-gen-mets-no-target.rb'

  VALID_TEXT          = 'test/fixtures/texts/valid-no-target'
  TEXT_WITH_TARGET    = 'test/fixtures/texts/valid'             # this should fail as "no-target" script should not be used with texts that targets
  ...
  CANONICAL_XML       = 'test/fixtures/canonical/valid_mets-no-target.xml'
  ...
  def test_exit_status_with_valid_text
    ...
    assert(s.exitstatus == 0, "incorrect exit status: error: #{e}")
