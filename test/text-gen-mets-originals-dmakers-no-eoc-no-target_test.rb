...
class TestTextGenMetsOriginalsDmakersNoEocNoTarget < MiniTest::Test

  COMMAND = 'ruby bin/text-gen-mets-originals-dmakers-no-eoc-no-target.rb'

  VALID_TEXT          = 'test/fixtures/texts/valid-originals-dmakers-no-eoc-no-target'
  ...
  BAD_O_D_COUNT_TEXT  = 'test/fixtures/texts/bad-o-d-file-count'
  BAD_M_D_PREFIX_TEXT = 'test/fixtures/texts/bad-m-d-prefix'
  CANONICAL_XML       = 'test/fixtures/canonical/valid_mets-originals-dmakers-no-eoc-no-target.xml'

  def test_exit_status_with_text_that_has_eoc_file
    o, e, s = Open3.capture3("#{COMMAND} 'nyu_aco000003' 'SOURCE_ENTITY:TEXT' 'VERTICAL' 'LEFT_TO_RIGHT' 'RIGHT_TO_LEFT' #{TEXT_WITH_EOC}")
    assert(s.exitstatus == 1, "incorrect exit status. output: '#{o}' error: '#{e}'")
  end

  def test_exit_status_with_text_that_has_target_file
    o, e, s = Open3.capture3("#{COMMAND} 'nyu_aco000003' 'SOURCE_ENTITY:TEXT' 'VERTICAL' 'LEFT_TO_RIGHT' 'RIGHT_TO_LEFT' #{TEXT_WITH_TARGET}")
    assert(s.exitstatus == 1, "incorrect exit status. output: '#{o}' error: '#{e}'")
  end
  ...
  def test_exit_status_with_valid_text
    ...
    assert(s.exitstatus == 0, "incorrect exit status. output: '#{o}' error: '#{e}'")
  ...
  # def test_mismatched_original_dmaker_file_count
  #   o, e, s = Open3.capture3("#{COMMAND} 'nyu_aco000003' 'SOURCE_ENTITY:TEXT' 'HORIZONTAL' 'RIGHT_TO_LEFT' 'LEFT_TO_RIGHT' #{BAD_O_D_COUNT_TEXT}")
  #   assert(s.exitstatus != 0)
  #   assert(o == '')
  #   assert_match(/invalid slot list/, e)
  # end

  # def test_mismatched_original_dmaker_file_prefixes
