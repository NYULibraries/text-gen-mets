...
class TestTextGenMetsNoMarcxmlNoEocNoTarget < MiniTest::Test

  COMMAND = 'ruby bin/text-gen-mets-no-marcxml-no-eoc-no-target.rb'

  VALID_TEXT          = 'test/fixtures/texts/valid-no-marcxml-no-eoc-no-target'
  ...
  TEXT_WITH_MARCXML   = 'test/fixtures/texts/valid-no-eoc-no-target'  # this should fail as "no-marcxml-no-eoc-no-target" script should not be used with texts that have marcxml files
  ...
  CANONICAL_XML       = 'test/fixtures/canonical/valid_mets-no-marcxml-no-eoc-no-target.xml'

  def test_exit_status_with_text_that_has_eoc_file
    _, e, s = Open3.capture3("#{COMMAND} 'nyu_aco000003' 'SOURCE_ENTITY:TEXT' 'VERTICAL' 'LEFT_TO_RIGHT' 'RIGHT_TO_LEFT' #{TEXT_WITH_EOC}")
    assert(s.exitstatus == 1, "incorrect exit status")
    assert_match(/EOC FILE DETECTED/, e, 'incorrect EOC error message')
  end

  def test_exit_status_with_text_that_has_target_file
    _, e, s = Open3.capture3("#{COMMAND} 'nyu_aco000003' 'SOURCE_ENTITY:TEXT' 'VERTICAL' 'LEFT_TO_RIGHT' 'RIGHT_TO_LEFT' #{TEXT_WITH_TARGET}")
    assert(s.exitstatus == 1, "incorrect exit status")
    assert_match(/TARGET FILE DETECTED/, e, 'incorrect TARGET error message')
  end

  def test_exit_status_with_text_that_has_marcxml_file
    _, e, s = Open3.capture3("#{COMMAND} 'nyu_aco000003' 'SOURCE_ENTITY:TEXT' 'VERTICAL' 'LEFT_TO_RIGHT' 'RIGHT_TO_LEFT' #{TEXT_WITH_MARCXML}")
    assert(s.exitstatus == 1, "incorrect exit status")
    assert_match(/MARCXML FILE DETECTED/, e, 'incorrect MARCXML error message')
  end
