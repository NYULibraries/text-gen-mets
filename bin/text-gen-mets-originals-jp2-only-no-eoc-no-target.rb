#------------------------------------------------------------------------------
# Time-stamp: <2017-08-07 16:31:11 jgp>
#------------------------------------------------------------------------------
...
def emit_struct_map_slot_div(slot, order)
  ...
  slot.dmakers.each do |d|
    puts "                <fptr FILEID=\"f-#{d.rootname_minus_role}_#{d.role_to_abbreviation}\"/> "
  end
  puts '            </div> '
end
...
def get_md_file_inventory(dir)
  ...
  # THIS SCRIPT SHOULD NOT BE USED IF TARGET FILE IS PRESENT
...
def get_original_files(dir)
  get_files(dir, '*_o.jp2', /.+_ztarget_m.tif/).collect { |f| Filename.new(f) }
end
