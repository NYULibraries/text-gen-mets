#------------------------------------------------------------------------------
# Time-stamp: <2019-10-13 18:40:34 jgp>
#------------------------------------------------------------------------------
#
# NOTE: THIS CODE ASSERTS THAT AN EOC    FILE IS ***NOT*** PRESENT
# NOTE: THIS CODE ASSERTS THAT A  TARGET FILE IS ***NOT*** PRESENT
#
# script generates METS files for NYU DLTS text objects with 
# "original" (_o.jp2) and "dmaker" (_o.jp2) files.
#
# invocation:
...
def emit_file_grp_original_open
  puts %(        <fileGrp ID="fg-original" USE="ORIGINAL">)
end
...
def emit_file(fname)
  match = /(.+)\.jp2\z/.match(fname)
  raise "badly formed filename #{fname}" unless match
  id = match[1]
  puts %(            <file ID="f-#{id}" MIMETYPE="image/jp2">)
...
def get_md_file_inventory(dir)
  ...
  # THIS SCRIPT SHOULD NOT BE USED IF EOC FILE IS PRESENT
  result = Dir.glob(File.join(dir, "*_eoc.csv"))
  if result.length != 0
...
def validate_and_extract_args(args_in)
  ...
  # assemble file lists
#  original_files = get_original_files(args_out[:dir])
  dmaker_files   = get_original_files_as_dmakers(args_out[:dir])

  slot_list = ''
  args = OpenStruct.new
#  args.originals  = original_files
...
#  args_out[:original_files] = original_files
...
# def get_original_files(dir)
#   get_files(dir, '*_o.jp2').collect { |f| Filename.new(f) }
# end
...
def get_original_files_as_dmakers(dir)
  get_files(dir, '*_o.jp2').collect { |f| Filename.new(f, role: :dmaker) }
end
...
emit_amd_sec_close
emit_file_sec_open
# emit_file_grp_original_open
# emit_files(args[:original_files])
# emit_file_grp_close
